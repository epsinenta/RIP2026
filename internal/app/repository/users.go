...
func (r *Repository) EditInfo(id int, j serializer.UserJSON) (ds.Users, error) {
	if id <= 0 {
	...
	updates := serializer.UserFromJSON(j)
	if updates.IsModerator && !currentUser.IsModerator {
		updates.IsModerator = false
	}
	if err := r.db.Model(&currentUser).Updates(updates).Error; err != nil {
